...
static bool checkTupleLikeDecomposition(Sema &S,
  ...
  for (auto *B : DD->flat_bindings()) {
    ...
    // Expansion statements synthesize a constexpr decomposition of the range
    // whose unnamed bindings are read by value during constant evaluation. Do
    // not make those bindings constexpr: that would (as for a user-written
    // non-static constexpr structured binding of a class type) require the
    // referenced subobjects to have constant addresses, which a local range
    // object does not. The element values remain constant via the constexpr
    // decomposition itself.
    if (!isa<ExpansionStmtDecl>(Src->getDeclContext()))
